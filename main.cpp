...
void imprime_let_dig(char letras[26],int tam){
    cout<<"\n\nLetras digitadas: ";
    for(int i=0; i < tam;i++){
        cout<< letras[i]<<" , ";
...
int main()
{
    ...
    limpa_palavra_dig(); // limpa o vetor da palavra digitada

    int cont_let_dig=0;

    int cont_erros=0;

    char letras_digitados[26];
    char letra;
    ...
    imprime_palavra_dig();
    imprime_let_dig(letras_digitados,cont_let_dig);
    ...
    if(verifica_letra(letra,letras_digitados)){
        cout<<"Letra ja foi digitada! Tente novamente! \n\n";
        system("pause");
    }else{
        letras_digitados[cont_let_dig] = letra;
        cont_let_dig++;
